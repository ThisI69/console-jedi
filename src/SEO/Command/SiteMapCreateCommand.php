...
namespace Notamedia\ConsoleJedi\SEO\Command;
 ...
 * @subpackage main
 */
class SiteMapCreateCommand extends BitrixCommand
{
    protected function configure()
    {
        $this
            // the name of the command (the part after "bin/console")
            ->setName('seo:sitemapcreate')

            // the short description shown while running "php bin/console list"
            ->setDescription('Create sitemap.xml')

            // the full command description shown when running the command with
            // the "--help" option
            ->setHelp('This system command create sitemap.xml')
            ->addArgument(
                'ID',
            ...
            if ($v == $arValueSteps['init']) {
                ...
                while (!$bFinished && microtime(true) <= $ts_finish) {
                    ...
                    if ($arRes = $dbRes->Fetch()) {
                        $this->seoSitemapGetFilesData($PID, $arSitemap, $arRes, $sitemapFile);
                        $bCheckFinished = false;
